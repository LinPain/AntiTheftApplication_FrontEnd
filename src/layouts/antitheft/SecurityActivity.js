  ...
  const getEventIcon = (type) => {
    switch (type) {
      case "LOST_MODE_ENABLED":
        return { icon: "lock", color: colors.dangerSoft };
      case "LOST_MODE_DISABLED":
        return { icon: "lock_open", color: colors.teal };
      case "ALARM_STARTED":
      case "ALARM_STARTED_ON_DEVICE":
        return { icon: "notifications_active", color: colors.warning };
      case "ALARM_STOPPED":
      case "ALARM_STOPPED_ON_DEVICE":
        return { icon: "notifications_off", color: colors.cyan };
      case "DEVICE_OFFLINE":
        return { icon: "cloud_off", color: colors.textMuted };
      case "DEVICE_RECONNECTED":
        return { icon: "cloud_done", color: colors.teal };
      case "SIM_CHANGED":
        return { icon: "sim_card_alert", color: colors.dangerSoft };
      case "FAILED_PIN_ATTEMPT":
        return { icon: "gpp_maybe", color: colors.warning };
      default:
        return { icon: "info", color: colors.cyan };
    }
  };
      ...
      <Stack spacing={2.5}>
        <Stack direction="row" justifyContent="space-between" alignItems="flex-end">
          <Stack spacing={0.75}>
            ...
            <Typography variant="body2" sx={{ color: colors.textSecondary }}>
              Lịch sử các sự kiện an ninh và phản hồi từ hệ thống.
            </Typography>
          </Stack>
        ...
        <SentinelSurface elevation={0} sx={{ p: 2.5, minHeight: "600px" }}>
          {isLoading ? (
            ...
            <Typography
              variant="body2"
              sx={{ color: colors.textSecondary, textAlign: "center", py: 5 }}
            >
                  ...
                  <Box key={event._id}>
                    <Stack direction="row" spacing={2} alignItems="center">
                      ...
                      <Stack spacing={0.5} sx={{ flex: 1 }}>
                        <Typography
                          variant="body2"
                          sx={{ color: colors.white, fontWeight: 600, fontFamily: fonts.mono }}
                        >
                          {event.eventType.replace(/_/g, " ")}
                        </Typography>
                        <Typography variant="caption" sx={{ color: colors.textSecondary }}>
                          {new Date(event.timestamp).toLocaleString("vi-VN")} •{" "}
                          {event.details || "System automated event"}
                        </Typography>
                      </Stack>
                    </Stack>
