...
function Devices() {
  ...
  const selectedDevice = useMemo(
    () => deviceList.find((d) => d._id === selectedId) || null,
    [selectedId, deviceList]
  );
  ...
  const handleLostMode = async (active) => {
    const msg = active
      ? prompt(
          t("lostModeMessagePrompt", "Nhập thông điệp báo mất:"),
          "Thiết bị này đã bị báo mất!"
        )
      : "";
    const phone = active
      ? prompt(t("lostModePhonePrompt", "Nhập số điện thoại liên hệ:"), user.phone)
      : "";
        ...
        >
          <Stack spacing={0.75}>
            <Typography variant="caption" sx={{ ...textStyles.label, color: colors.cyan }}>
              06 / {t("deviceRegistry").toUpperCase()}
            </Typography>
            <Typography
              component="h1"
              variant="h4"
          ...
          <Button
            variant="outlined"
            startIcon={<IconGlyph name="refresh" size={17} />}
          ...
          {selectedDevice ? (
            <SentinelSurface component="section" elevation={0} sx={{ flex: 1, p: 2.5 }}>
              ...
              >
                <Stack direction="row" spacing={1.5} alignItems="center">
                  <BoxIcon
                    name={selectedDevice.deviceName?.includes("Web") ? "laptop" : "phone_android"}
                  />
                  <Stack spacing={0.5}>
                    <Typography variant="h6" sx={{ color: colors.white, fontWeight: 600 }}>
                      {selectedDevice.deviceName || selectedDevice._id}
                    </Typography>
                    <Typography
                      variant="caption"
                      sx={{ ...textStyles.mono, color: colors.textMuted }}
                    >
                      {selectedDevice.manufacturer} {selectedDevice.model} ·{" "}
                      {selectedDevice.androidVersion || "Web"}
                    </Typography>
                  </Stack>
                </Stack>
                <Chip
                  label={
                    new Date() - new Date(selectedDevice.lastTimestamp) < 300000
                      ? "ONLINE"
                      : "OFFLINE"
                  }
                  size="small"
              ...
              <Stack direction={{ xs: "column", sm: "row" }} spacing={2}>
                ...
                <DeviceMetric
                  label="IP ADDRESS"
                  value={selectedDevice.ipAddress || "N/A"}
                  icon="lan"
                />
                <DeviceMetric
                  label={t("risk", "RỦI RO").toUpperCase()}
                  value={`${selectedDevice.riskScore || 0} / 100`}
                  icon="gpp_maybe"
                />
              </Stack>
              ...
              <Stack spacing={1.5} sx={{ mt: 3 }}>
                <DetailRow
                  label={t("status", "TRẠNG THÁI").toUpperCase()}
                  value={
                    globalStatus.lostMode?.active
                      ? t("lostModeActive", "ĐÃ BÁO MẤT (LOCKED)")
                      : t("statusVerified")
                  }
                  icon="lock"
                />
                <DetailRow
                  label={t("lastUpdate", "CẬP NHẬT CUỐI").toUpperCase()}
                  value={new Date(selectedDevice.lastTimestamp).toLocaleString(
                    language === "vi" ? "vi-VN" : "en-US"
                  )}
                  icon="schedule"
                />
                <DetailRow
                  label={t("deviceId").toUpperCase()}
              ...
              <Stack direction="row" spacing={2}>
                <Button
                  ...
                  startIcon={
                    <IconGlyph name={globalStatus.lostMode?.active ? "lock_open" : "lock"} />
                  }
                  sx={{ borderRadius: 0, flex: 1, color: "white" }}
                >
                  {globalStatus.lostMode?.active
                    ? t("unlockDevice", "Mở khoá từ xa")
                    : t("lockDevice")}
                </Button>
                <Button
                  variant="outlined"
                  ...
                  startIcon={
                    <IconGlyph
                      name={
                        globalStatus.alarm?.active ? "notifications_off" : "notifications_active"
                      }
                    />
                  }
                ...
                <IconButton
                  color="error"
                  onClick={() => handleRemove(selectedDevice._id)}
                  sx={{ border: `1px solid ${colors.dangerSoft}`, borderRadius: 0 }}
                >
            ...
            >
              <Typography color="textSecondary">
                {t("selectToManage", "Chọn một thiết bị để xem chi tiết.")}
              </Typography>
            </SentinelSurface>
          )}
        </Stack>
...
function BoxlessDeviceList({ devices, selectedId, onSelect, t }) {
  return (
    <SentinelSurface
      component="section"
      elevation={0}
      sx={{ width: { xs: "100%", xl: "360px" }, flexShrink: 0, p: 2 }}
    >
      <Typography variant="caption" sx={{ ...textStyles.label, color: colors.cyan }}>
        DATABASE / {devices.length.toString().padStart(2, "0")}
      </Typography>
        ...
        {devices.map((device) => (
          ...
          >
            <Stack direction="row" spacing={1.25} alignItems="center" sx={{ width: "100%" }}>
              <IconGlyph
                name={device.deviceName?.includes("Web") ? "laptop" : "phone_android"}
                color={selectedId === device._id ? colors.cyan : colors.textMuted}
              />
              <Stack spacing={0.35} sx={{ minWidth: 0 }}>
                ...
                <Typography variant="caption" sx={{ ...textStyles.mono, color: colors.textMuted }}>
                  {new Date(device.lastTimestamp).toLocaleTimeString()} · {device.batteryLevel}%
                </Typography>
              </Stack>
            </Stack>
          </Button>
...
function BoxIcon({ name }) {
  return (
    ...
    >
      <IconGlyph name={name} size={25} />
    </Stack>
  );
}
function DeviceMetric({ label, value, icon }) {
  return (
    <SentinelSurface elevation={0} sx={{ flex: 1, p: 1.5, backgroundColor: `${colors.void}8C` }}>
      <Stack direction="row" spacing={1} alignItems="center">
        <IconGlyph name={icon} size={16} color={colors.cyan} />
        <Typography variant="caption" sx={{ ...textStyles.label, color: colors.textMuted }}>
          {label}
        </Typography>
      </Stack>
      <Typography variant="body2" sx={{ mt: 1, color: colors.white, fontFamily: fonts.mono }}>
        {value}
      </Typography>
    </SentinelSurface>
  );
...
function DetailRow({ label, value, icon }) {
  return (
    <Stack
      direction={{ xs: "column", sm: "row" }}
      spacing={{ xs: 0.5, sm: 2 }}
      sx={{ borderBottom: `1px solid ${colors.line}`, pb: 1.25 }}
    >
