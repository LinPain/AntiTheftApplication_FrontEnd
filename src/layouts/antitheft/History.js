  ...
  const fetchHistory = async (id) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_BASE}/api/${user.username}/location/${id}`,
        {
...
const selectSx = {
  ...
  "&:hover .MuiOutlinedInput-notchedOutline": { borderColor: `${colors.cyan}80` },
  "&.Mui-focused .MuiOutlinedInput-notchedOutline": { borderColor: colors.cyan },
  "& .MuiSvgIcon-root": { color: colors.cyan },
};
  ...
  return (
    <Stack direction="row" spacing={1.5} alignItems="flex-start">
      <Stack alignItems="center" sx={{ width: "16px", flexShrink: 0 }}>
        ...
        />
        <Box
          sx={{ width: "1px", flex: 1, minHeight: "42px", mt: 1, backgroundColor: colors.line }}
        />
      </Stack>
      <Box sx={{ flex: 1, minWidth: 0, borderBottom: `1px solid ${colors.line}`, pb: 1.5 }}>
        ...
        <Typography variant="body2" sx={{ mt: 0.75, color: colors.textPrimary, fontWeight: 600 }}>
